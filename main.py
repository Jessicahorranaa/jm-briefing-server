...
@app.get("/")
def health():
    return {"status": "JM Studio Briefing Visual v3 - online"}

@app.post("/webhook")
async def webhook(dados: str = Form(...)):
    try:
        d = json.loads(dados)
        print(f"[BRIEFING] {d.get('nome','?')} | {d.get('nicho','?')}")
...
def gerar_diagnostico(d):
    if not GROQ_API_KEY:
        return "Groq nao configurado. Adicione GROQ_API_KEY nas variaveis de ambiente do Render."
    cabelo = f"{d.get('cabelo_comprimento','')} {d.get('cabelo_textura','')} {d.get('cabelo_cor','')}".strip()
    refs = (f"Ref1: {d.get('ref1_link','')} - {d.get('ref1_desc','')} | "
            f"Ref2: {d.get('ref2_link','')} - {d.get('ref2_desc','')} | "
            f"Ref3: {d.get('ref3_link','')} - {d.get('ref3_desc','')}")
    prompt = f"""Voce e Jessica Marques, diretora criativa de ensaio fotografico IA para marca pessoal feminina premium.
Crie um DIAGNOSTICO ESTRATEGICO VISUAL completo com 5 secoes bem desenvolvidas:
1. LEITURA DA MARCA - Quem e essa mulher, onde esta posicionada, o que sua marca precisa comunicar.
2. DIRECAO CRIATIVA - Conceito visual: luz, atmosfera, cenario ideal, paleta, mood board em palavras.
3. LOOKS E STYLING - Sugestoes de roupas, cores, acessorios e calcados que funcionam para ela.
4. POSES E EXPRESSOES - Orientacoes de postura, angulos, expressoes e o que valorizar.
5. PROXIMOS PASSOS - Recomendacoes praticas para execucao do ensaio.
BRIEFING: Nome: {d.get('nome','')} | Nicho: {d.get('nicho','')} | Instagram: {d.get('instagram','')} | Estagio: {d.get('estagio','')}
Objetivo visual: {d.get('objetivo_visual','')}
Sensacoes: {d.get('sensacoes','')} | Percepcao: {d.get('percepcao_principal','')} | Frase: {d.get('frase_imagem','')}
Nao percepcao: {d.get('nao_percepcao','')} | Publico sentir: {d.get('pub_sentir','')} | Objetivo fotos: {d.get('obj_fotos','')}
Nao transmitir: {d.get('nao_transmitir','')} | Nao representa: {d.get('nao_representa','')}
...
Tons: {d.get('tons','')} | Looks: {d.get('looks','')} | Nao look: {d.get('nao_look','')}
Acessorios: {d.get('acessorios','')} | Calcados: {d.get('calcados','')} | Nao acessorios: {d.get('nao_acessorios','')}
Estilo: {d.get('estilo_ideal','')} | Refs: {refs} | Estilo admira: {d.get('ref_estilo','')}
Obs: {d.get('obs_finais','')} | Fotos: {d.get('fotos_enviadas','')}
Escreva em portugues, sofisticado. Minimo 500 palavras."""
...
def salvar_notion(d, diag):
    ...
    titulo = f"Briefing Visual {nome} - {datetime.now().strftime('%d/%m/%Y')}"
    tom = d.get("tom_pele", "")
    ...
    blocos += [db(), hb("Dados do Briefing"), db(),
        tb(f"Nome: {nome} | Nicho: {d.get('nicho','')} | Instagram: {d.get('instagram','')} | Estagio: {estagio}"),
        tb(obj_v[:1900]), tb(nao_t[:1900]), tb(f"Altura: {d.get('altura','')} | Peso: {d.get('peso','')} | Tom: {tom} | Corpo: {d.get('corpo','')}"),
        tb(tracos_f[:1900]), tb(looks_f[:1900]), tb(calc_f[:1900]), tb(refs[:1900]),
        tb(f"Estilo: {d.get('estilo_ideal','')} | Obs: {d.get('obs_finais','')}")]
